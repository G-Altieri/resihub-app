...
export default function DispositiviPage() {
  ...
  // Filtra i Dispositivi di tipo "Dispositivi"
  const devices = parsedData.dispositivi.filter(
    (device: any) => device.tipo === 'Dispositivi'
  );

  return (
    <View style={styles.container}>
      <Image
        source={require('../../../assets/images/SfondoDispositivi.jpg')}
        style={StyleSheet.absoluteFill}
        resizeMode="contain"
      />
      <Text style={styles.title}>Dispositivi di Dispositivi</Text>
      {devices.length === 0 ? (
        <Text style={styles.infoText}>Nessun dispositivo trovato per Dispositivi</Text>
      ) : (
        <FlatList
          data={devices}
